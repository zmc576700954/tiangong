...
interface GraphState {
  ...
  createGraph: (name: string, type: 'production' | 'development', sourceGraphId?: string) => Promise<Graph>
...
export const useGraphStore = create<GraphState>((set, get) => ({
  ...
  loadGraph: async (graphId: string) => {
    ...
    if (result) {
      ...
      const allBugs = await window.electronAPI["'bug:listByGraph'"](graphId)
      set({ bugs: allBugs })
    }
  },

  createGraph: async (name, type, sourceGraphId) => {
    const graph = await window.electronAPI["'graph:create'"]({ name, type, sourceGraphId })
  ...
  setCurrentGraph: (id) => {
    set({ currentGraphId: id, selectedNodeId: null })
    if (id) { get().loadGraph(id) } else { set({ nodes: [], edges: [], bugs: [] }) }
  },
  ...
  updateEdge: async (id, data) => {
    ...
    set((state) => ({ edges: state.edges.map((e) => (e.id === id ? updated : e)) }))
  },
  ...
  updateBug: async (id, data) => {
    ...
    set((state) => ({ bugs: state.bugs.map((b) => (b.id === id ? updated : b)) }))
  },
