...
export class MemoryExtractor {
  ...
  classifyOutput(outputs: AgentOutput[]): OutputHealth {
    ...
    const truncationMarkers = [
      /\.\.\.\s*$/,                    // 省略号结尾
      /(?:truncat|limit|exceed)/i,     // 截断/限制关键词
      /\[\s*$/,                         // 未闭合的方括号
      /\(\s*$/,                         // 未闭合的圆括号
      /(?:token|context).*(?:limit|budget|exhausted)/i,  // Token/上下文耗尽
    ]
    if (truncationMarkers.some((m) => m.test(fullText.substring(-200)))) {
      return 'truncated'
    }
    ...
    if (poisonMarkers.some((m) => m.test(fullText.substring(-500)))) {
      return 'poisoned'
    }
  ...
  private _extractDecisions(text: string): string[] {
    ...
    const decisionHints = [
      /(?:decided|chose|opted|went)\s+(?:to|for|with)\s+(.+?)(?:\.|because|since|as|$)/gi,
      /(?:decision|选择|决定)[:：]\s*(.+)/gi,
      /(?:use|using|switch to|migrate to)\s+(?:the\s+)?([A-Z][a-zA-Z.]{2,40}(?:\s+[A-Z][a-zA-Z.]{2,40})*)(?:\.|\s+because|\s+since|\s+as|$)/gi,
      /(?:recommend|suggest)\s+(?:using|to use|adopting)\s+(.+?)(?:\.|because|since|as|$)/gi,
      /(?:over|instead of|rather than)\s+(.+?)(?:\.|because|since|as|$)/gi,
      /(?:选择|采用|迁移到|替换为)\s*(.+?)(?:[。，,.\n]|$)/gi,
    ]
  ...
  private _extractLessons(text: string): string[] {
    ...
    const lessonHints = [
      /(?:learned|realized|noticed|found)\s+that\s+(.+?)(?:\.|$)/gi,
      /(?:important|critical|crucial|key)\s+(?:to )?(?:note|remember|know|understand)[:：]?\s+(.+?)(?:\.|$)/gi,
      /(?:watch out for|beware of|be careful with)\s+(.+?)(?:\.|$)/gi,
      /(?:教训|经验|踩坑|注意|坑)[:：]\s*(.+)/gi,
      /(?:避免|不要|小心|谨慎)\s*(.+?)(?:[。，,.\n]|$)/gi,
      /(?:下次|以后|更好的做法是)\s*(.+?)(?:[。，,.\n]|$)/gi,
      /(?:turns out|it appears that|the issue was|the problem was)\s+(.+?)(?:\.|$)/gi,
    ]
